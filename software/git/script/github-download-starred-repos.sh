...
if [ -z "$GITHUB_READ_ONLY_TOKEN" ]; then
        printf '%s: GITHUB_READ_ONLY_TOKEN envirnoment variable not set. Please provide read only api token for GitHub\n' "$0" >&2
        exit 1
fi
...
while [ -n "$github_starred_repos_endpoint" ]; do
        # The full response
        response="$(curl -s -H "Authorization: token $GITHUB_READ_ONLY_TOKEN" "$github_starred_repos_endpoint")"
        ...
        github_starred_repos_endpoint="$(curl -sI -H "Authorization: token $GITHUB_READ_ONLY_TOKEN" "$github_starred_repos_endpoint" | grep -i '^Link:' | sed -n 's/.*<\([^>]\+\)>;[[:space:]]*rel="next".*/\1/p')"
done
